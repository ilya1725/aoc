...
public:
  ...
  const unsigned kMinIndx = 0;
  const unsigned kMeanIndx = 1;
  const unsigned kMaxIndx = 2;
  const unsigned kVarianceIndx = 3;
  const unsigned kCntIndx = 4;
  ...
  bool sample(uint32_t id, double value) {
    ...
    if (range[kCntIndx] == 0.0) {
      ...
      auto delta1 = value - range[kMeanIndx];
      // avg
...
inline std::ostream& operator<<(std::ostream& os, const TimeMsr& tm) {
  ...
  for (auto & m : tm.GetOrderedDataValues()) {
    os << m.first << "|" << m.second[0] << ";"
                         << m.second[1] << ";"
                         << m.second[2] << ";"
                         << (m.second[3]/(m.second[4]-1)) << "|";
  }
  return os;
}
